...
- Wrapper flags: `--yolo`, `--ephemeral`, `--ro`, `--aws`, `--gh`, `--glab`, `--egress-allowlist`, `--iterm`, `--tmux`, `--claude-dir`, `-h`/`--help`.
...
- The `CLAUDE_DOCKER_EGRESS` and `CLAUDE_DOCKER_EGRESS_ALLOW` environment variables and their relationship to `--egress-allowlist`.
...
- **THEN** the output contains each of `--yolo`, `--ephemeral`, `--ro`, `--aws`, `--gh`, `--glab`, `--egress-allowlist`, `--iterm`, `--tmux`, `--claude-dir`, `-h`, `--help`, `--`, `CLAUDE_DOCKER_TMUX`, `CLAUDE_DOCKER_CONFIG_DIR`, `CLAUDE_DOCKER_EGRESS`, `CLAUDE_DOCKER_EGRESS_ALLOW`, and `settings.docker.json`
