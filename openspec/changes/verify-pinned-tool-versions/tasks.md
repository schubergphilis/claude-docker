...
- [x] 1.1 Convert `TOOLS` in `update_pins.py` from `(name, kind, ref)` tuples to a
  ...
  71 tests still pass
- [x] 1.2 Give each of the seven records a `probe` (whitespace-separated argv, e.g.
...
- [x] 2.1 Add a `--list-tools` early-return mode emitting one TSV row per tool
...
- [x] 2.2 Make the producer fail closed on the same contract as `--list-npm-tools`:
  ...
  unit test in 3.2
- [x] 2.3 Wire the flag through `parse_args` and `main`'s early-return ordering
...
- [x] 3.1 Cover the listing: exactly seven rows, tool names in registry order, four
  tab-separated columns, and each row's version equal to the value in its `pins/*.env`
  fragment; verify the tests pass
- [x] 3.2 Cover the fail-closed path: a tool with an empty version pin makes
...
- [x] 3.3 Cover the rule invariants every entry must hold: exactly one capture group,
...
- [x] 3.4 Cover registry completeness: every tool in the registry has a non-empty
...
- [x] 4.1 Replace `ci.yml`'s `Smoke test — claude --version matches Dockerfile pin` step
...
- [x] 4.2 Hold the step to the existing audit step's fail-closed shape: `set -euo
...
- [x] 4.3 Write each tool's whole check as one conditional chain — `if out=$(docker run
...
- [x] 4.4 Echo each tool's name, pinned version, and reported version on every run
...
- [x] 4.5 Keep the step correct by construction — split `probe` with `read -ra` into an
...
- [x] 5.1 Describe `--list-tools` in `README.md`'s pinned-tool-versions section as prose
...
- [x] 6.1 Run `python3 -m unittest discover -s tests -p 'test_*.py' -v` and confirm the
  full suite passes with the new tests included
- [x] 6.2 Prove the shared-rule assumption without an image: feed each tool's recorded
...
- [x] 6.3 Run `openspec validate verify-pinned-tool-versions --type change --strict
