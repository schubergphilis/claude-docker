...
- [ ] 1.1 Carry `pins-updater.yml`'s 2026-09-14 refresh commit onto this branch with its
...
- [ ] 2.1 After the existing `claude-code` `install.cjs` line in the same `RUN`, invoke
...
- [ ] 2.2 Assert in the same `RUN`, immediately after, that the first four bytes of
  ...
  runtime*)
- [ ] 2.3 Extend the comment above the npm layer to cover the second script: state what
...
- [ ] 2.4 Confirm no separate `npm install` invocation without `--ignore-scripts` was
...
- [ ] 3.1 Add `pn` and `pnx` to the "Bundled CLIs on the default PATH" line in `README.md`
  (spec: *bundled CLIs list includes new tools*)
- [ ] 3.2 Name `pnpx`/`pnx` as aliases of `pnpm dlx` in the Threat model's runtime code-fetch
...
- [ ] 4.1 `openspec validate link-pnpm-native-binary --strict` passes
- [ ] 4.2 `python3 -m unittest discover -s tests -p 'test_*.py'` passes (no pin-tooling change
  here, but the pins moved)
- [ ] 4.3 Verify the install-then-link sequence against the real 12.3.4 package outside the
...
- [ ] 4.4 Leave the `docker build` + smoke matrix to CI, which is also what exercises the
