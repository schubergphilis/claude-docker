...
- [ ] 2.1 Resolve `aquasecurity/trivy-action`'s newest release to its commit SHA and add both
...
- [ ] 3.1 Create `.github/workflows/image-scan.yml` following `pins-updater.yml`'s shape:
  ...
  * * 1` (one hour after the pins refresh, offset from Dependabot's window),
  ...
  (spec: *on-demand scan*)
- [ ] 3.2 Give the job a checkout with `persist-credentials: false`, a buildx setup, and a
...
- [ ] 4.4 Confirm the README's pinned-tool-versions section still reads correctly beside the
