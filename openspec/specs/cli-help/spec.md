...
Let users discover every `claude-docker` wrapper flag directly from the terminal instead of reading `run.sh` or the README. `-h`/`--help` prints a self-contained per-flag reference and exits before any Docker or filesystem side effects, while preserving the `--` passthrough contract so `claude-docker -- --help` still forwards `--help` to `claude`.
...
- Wrapper flags: `--yolo`, `--ephemeral`, `--ro`, `--aws`, `--gh`, `--glab`, `--api`, `--az`, `--iterm`, `--tmux`, `--claude-dir`, `-h`/`--help`.
