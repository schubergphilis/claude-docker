...
`ccusage` is preinstalled for statuslines that show token usage or spend. It reads the container's own session transcripts in the `claude-code-home` volume, not the host's, so it reports container sessions only. By default it fetches a public model-pricing table over the network; `ccusage --offline` uses the prices bundled with the pinned version.
