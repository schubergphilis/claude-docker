...
| `--egress-allowlist` | Default-deny network egress through a per-session forward-proxy sidecar. See [Egress allowlist](#egress-allowlist). Sticky form: `CLAUDE_DOCKER_EGRESS=allowlist`. |

`--ro` does **not** block credential flags or restrict network egress. `--egress-allowlist` restricts egress. For an isolated review session, combine all three and pass no credential flags:
...
>
> The forwarded _env vars_ are tightly scoped (named individually), so the over-share is specific to the npmrc/pip.conf file mounts. To minimise exposure, prefer the env-var channel or keep registry-only config files, and remember the container has full network egress unless `--egress-allowlist` is set (see [Threat model](#threat-model)).
...
`--egress-allowlist` (or `CLAUDE_DOCKER_EGRESS=allowlist` in your shell rc) turns on **default-deny network egress** for the session. It is off by default. The design is in [`openspec/changes/add-egress-allowlist/design.md`](openspec/changes/add-egress-allowlist/design.md).

**How it works.**

- The agent container is attached **only** to a per-session `--internal` Docker network (`claude-egress-<id>`). That network has no route off the host, and external DNS does not resolve.
- The one way out is a per-session **squid forward proxy** sidecar (`claude-egress-proxy-<id>`). It runs from this same image (`--entrypoint squid`, as the unprivileged `proxy` user, all capabilities dropped).
- The agent gets `HTTP(S)_PROXY` pointing at it, and squid allows only allowlisted hosts on ports 80/443.
- TLS stays end-to-end (`CONNECT`), so the sidecar sees hostnames, never content, and there are no certificates to trust.
- The network, not the proxy, is the boundary. A tool that ignores the proxy env vars simply gets no connection.
- No capability is added to the agent container.

**What is allowed** is the union of these sources:

| Source | Hosts | Can name private / internal addresses? |
| --- | --- | --- |
| Built-in | `.anthropic.com`, `.claude.ai`, `.claude.com` (Claude Code's [required hosts](https://code.claude.com/docs/en/network-config#network-access-requirements); optional telemetry is not included) | no |
| `--gh` / `--gh-direct` | `github.com`, `api.github.com`, `uploads.github.com`, `codeload.github.com`, `.githubusercontent.com` | no |
| `--glab` | `.gitlab.com` | no |
| `--tfe` | `app.terraform.io` | no |
| `--aws` | `.amazonaws.com`, `.awsapps.com` | no |
| `CLAUDE_DOCKER_EGRESS_ALLOW` | yours: comma/space separated `host`, `.suffix` (domain and all subdomains), or `host:port` | **yes**, because you typed it on the host |
| `ANTHROPIC_BASE_URL` | the gateway host (and its port), when set on the host | **yes** |
| `<workspace>/.claude-docker/allowed-hosts` | the project's list, **once approved** (below) | no |

`--registry` adds nothing automatically. Add your registry host to `CLAUDE_DOCKER_EGRESS_ALLOW`. Self-hosted GitLab and anything a project fetches at runtime go there too, or in the project file: npm (`registry.npmjs.org`), PyPI (`pypi.org`, `files.pythonhosted.org`), Go (`proxy.golang.org`, `sum.golang.org`), `releases.hashicorp.com`.

**Per-project file.** `.claude-docker/allowed-hosts` in the first workspace holds one host per line; `#` comments and blank lines are allowed:
...
The file lives in the repo, so a repo you just cloned could try to grant itself egress. It is therefore a **proposal, not a grant**:

- Every line is validated strictly. Only hostname characters are allowed, IP literals and bare TLDs are rejected, and an invalid line aborts the run.
- The first time a list is seen (and after any change to its hosts), `run.sh` prints the hosts and asks `[y/N]`.
- An approval is remembered by the SHA-256 of the normalised list in `~/.config/claude-docker/egress-approved` (honours `XDG_CONFIG_HOME`). An unchanged list is re-approved silently.
- In a non-interactive run, an unapproved list is **ignored** with a warning, and the session starts without it.

**Always denied**, whatever the lists say:

- Cloud metadata and link-local (`169.254.0.0/16`, `fe80::/10`, `metadata.google.internal`, …). An `--aws` allowlist can therefore never reach IMDS.
- For every source except `CLAUDE_DOCKER_EGRESS_ALLOW` and `ANTHROPIC_BASE_URL`:
  - IP-literal targets;
  - names that resolve to private, loopback or CGNAT addresses. This is the DNS-rebinding defence, and it also blocks your LAN and the Docker host.
...
- **SSH is gone.** `git@github.com:…` remotes stop working; use HTTPS remotes (authenticated under `--gh`).
- **Allowlisted hosts can still carry data out.** For example, an attacker could use their own API key against `api.anthropic.com`, or push to their own repo on an allowlisted `github.com`. Hostname filtering narrows where data can go; it does not inspect what is sent.
- **DNS on old engines.** On Docker Engine older than 26, the embedded DNS still forwards external queries from internal networks, so DNS remains a covert channel there.
- **Docker only.** Podman is refused under this flag until its `--internal` semantics are verified.
- **Ports.** Only 80/443, plus any port given in `CLAUDE_DOCKER_EGRESS_ALLOW` / `ANTHROPIC_BASE_URL`. Such a port opens for every allowlisted host.
...
(singly and combined), `--ephemeral`, and `--ro`. One more cell, `--egress=1`, drives `run.sh --egress-allowlist` end to end. It proves that an allowlisted host is reachable through the proxy, and that a non-allowlisted host, the metadata IP, a raw `--noproxy` client and external DNS are all blocked. Most of the container's
