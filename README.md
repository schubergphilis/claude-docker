...
Run Claude Code in a container that inherits your setup but not your filesystem. Workspace access is scoped to the directories you pass in; your statusline, skills, agents, and slash commands ride along as read-only bind-mounts. CLI tools are preinstalled (`gh`, `glab`, `aws`, `az` with the `azure-devops` extension, `openspec`, `uv`, `pnpm`, `tfenv`, `git-lfs`, `task`) plus a version-pinned Go toolchain (`go`) — other language runtimes are not: `tfenv` and `uv` fetch your project-pinned Terraform / Python on demand. Host credentials (`gh`, `glab`, `aws`, `tfe`, `az`) are opt-in per flag; nothing leaks in by default.

The VCS and cloud CLIs (`gh`, `glab`, `aws`, `az`) need a flag to see host credentials — see [Credential opt-in](#credential-opt-in). The rest work out of the box.
...
**Credentials are off by default.** No AWS / GitHub / GitLab / Terraform Cloud / Azure DevOps / package-registry / model-endpoint config, tokens, or env vars reach the container unless you explicitly opt in:
...
| `--az`        | Azure DevOps (Services or on-prem Server): forward `AZURE_DEVOPS_EXT_PAT` (the PAT `az devops` / `repos` / `boards` / `pipelines` authenticate with) and `AZURE_DEVOPS_ORG_URL` (the default organization — the one place the hostname comes from, so a Server URL works as well as `dev.azure.com`), and mount `~/.azure/azureProfile.json` / `~/.azure/clouds.config` read-only when present. The host's Azure token caches (`msal_token_cache.json`, `accessTokens.json`) are **never** mounted, and there is no `gh auth token`-style discovery — `az` has no command that prints a PAT, so it comes from your environment. Without the flag, `/root/.azure/` is hidden by a tmpfs overlay. General Azure resource management (`az vm`, service principals, `ARM_*`) is out of scope: the image ships `azure-cli-core` + the `azure-devops` extension, not the full `azure-cli`. |
...
The two are complementary, not competing: nono's [container docs](https://nono.sh/docs/cli/internals/containers.md) recommend running it inside a container, which gives namespace isolation and resource limits while nono adds path-level filesystem control and credential blocking. To do that here, install nono in a child image (see [Extending the image](docs/workflows.md#extending-the-image)). nono is not built into the base image, for architectural reasons: its credential proxy runs inside the agent's own container, and its domain filtering needs `SYS_PTRACE`. See [#75](https://github.com/schubergphilis/claude-docker/issues/75) and [#12](https://github.com/schubergphilis/claude-docker/issues/12).
